...
COMPONENT IdealBattery IS_A OnePort
   "Ideal battery VOC(SOC)"
   DATA
      REAL Q = 2           UNITS u_Ah        "Nominal capacity"
   ...
   DECLS
      REAL soc             UNITS no_units    "State of charge (0..1)"
      REAL voc             UNITS u_V         "Open circuit voltage"
   INIT
      soc  = soc0
   CONTINUOUS
      -- SOC dynamics (simple coulomb counting)
      -- i < 0 discharge, i > 0 charge
      soc' =  ZONE (soc >=1) 0
              ZONE (soc <= 0) 0
              OTHERS  i / (Q*3600)
      -- Very simple linear OCV(SOC) relation
      voc = V_nom*linearInterp1D(V_SOC,max(0,min(1,soc)))
      -- Terminal voltage at pins of OnePort
      v = voc
END COMPONENT