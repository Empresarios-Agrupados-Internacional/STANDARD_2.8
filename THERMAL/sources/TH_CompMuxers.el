...
--------------------------------------------------------------------------------
-- Purpose: Inversion in order of the connection between two ports
--------------------------------------------------------------------------------
COMPONENT Th_InvIndex(
      INTEGER n = 1                                                     "Thermal port dimension")
      "Inverts the order of the thermal indexes"

   PORTS
      IN thermal( n ) tp_in
      OUT thermal( n ) tp_out

   CONTINUOUS
      EXPAND_BLOCK(i IN 1, n)
         tp_out.q[n + 1 - i] = tp_in.q[i]
         tp_out.Tk[n + 1 - i] = tp_in.Tk[i]
      END EXPAND_BLOCK
