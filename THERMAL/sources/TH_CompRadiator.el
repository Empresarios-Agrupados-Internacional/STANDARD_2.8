...
COMPONENT Radiator
   ...
   DATA
      ...
      INTEGER num   = 1             UNITS no_units                   "Number of parallel radiators considered"
   ...
   DECLS
      ...
      REAL A_tot                    UNITS u_m2        "Total area of radiator"
      REAL mass                     UNITS u_kg        "Total mass of radiator"
   ...
   CONTINUOUS
      ...
      EXPAND_BLOCK (i IN 1, nx)
         EXPAND_BLOCK (j IN 1, ny)
            ...
            k_var[i,j]  =  ZONE (TemperatureDependance)
                              FunVarSolidProp(mat, Conductivity, T[i,j], k, icpk[i,j])
                           OTHERS
                              FunVarSolidProp(mat, Conductivity, To, k, it0)

            cp_var[i,j] =  ZONE (TemperatureDependance)
                              FunVarSolidProp(mat, SpecificHeat, T[i,j], cp, icpk[i,j])
                           OTHERS
                              FunVarSolidProp(mat, SpecificHeat, To, cp, it0)
      ...
      q_tot_rad_1 = n_fin * sum2D(nx, ny, q_rad_1) * num

      -- Total heat flow radiated by side 2
      q_tot_rad_2 = n_fin * sum2D(nx, ny, q_rad_2) * num

      -- Total heat flow received from connection by radiator
      q_tot_fluid = n_fin * sum2D(1, ny, qx) * num
      ...
      -- Total radiator mass
      mass = rho_var * A_tot * e
      A_tot = A * num * n_fin * 2 -- Two faces
END COMPONENT
