...
-----------------------------------------------------------------------------------------------------------------------------------------------------------
-- Purpose:
--    Defines a vectorized diffusive thermal node calculating n different temperatures with no heat exchange between them
--
-----------------------------------------------------------------------------------------------------------------------------------------------------------
COMPONENT DNode_vec IS_A Node
	"Time dependant boundary condition in temperature (signal input)"
	DATA
		REAL To = 290	UNITS u_K	"Initial temperature"
		REAL C = 500	UNITS u_J_K	"Heat capacity"
	DECLS
	...
	INIT
		...
		END FOR
	CONTINUOUS
		EXPAND (i IN 1, n) T[i]' =	tp_in.q[i] / (C/n)

		-- Port variables 
		EXPAND (i IN 1, n) tp_in.Tk[i] = T[i]
...
COMPONENT GL_vec IS_A AGL
   "Linear thermal conductance with variable properties along the nodes"
   DATA
      REAL cond[n] = 0.1   UNITS u_W_K    "Total thermal conductance, vectorial"
   CONTINUOUS
      -- Conductance is distributed along the port connections
      EXPAND (i IN 1, n)
         Vcond[i] = cond[i]
...
--------------------------------------------------------------------------------
-- Purpose:
--    Convert a connection with size n to size m assuming same temperature and sum of heat fluxes
--------------------------------------------------------------------------------
COMPONENT DNodenTom IS_A DNode(
      INTEGER m = 1                       "Outlet port dimension")
      "Conversion from n-sized to m-sized thermal ports"

   PORTS
      OUT thermal( m ) tp_out

   CONTINUOUS
      <:q1>   q = qi + SUM(i IN 1,n; tp_in.q[i]) - SUM(i IN 1,m; tp_out.q[i])
      EXPAND( i IN 1, m ) tp_out.Tk[i] = T
